module UsersHelper
  ...
  def users_from_same_building
    User.where(building_id: current_user.building_id).where.not(id: current_user.id).pluck(:pseudo, :id)
  end

  def users_that_can_be_add_in_channel
    users_already_in = @channel.users.pluck(:id)
    User.where(building_id: current_user.building_id).where.not(id: users_already_in).pluck(:pseudo, :id)
  end
